open Ecs

module Collision_system = System.Make(Collision)
module Draw_system = System.Make(Draw)
module Move_system = System.Make(Move)
module Door_transition_system = System.Make(Door_transition)
module Dialogue_system = System.Make(Dialogue_system)
module Tutorial_system = System.Make(Tutorial_system)
module Library_cinematic_system = System.Make(Library_cinematic_system)